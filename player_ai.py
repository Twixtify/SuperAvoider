...
class PlayerAI(pygame.sprite.Sprite):
    ...
    def __init__(self, start_pos=(50, 50), game_display=pygame.Surface):
        ...
        self.number = PlayerAI.total_number
        PlayerAI.total_number += 1
        PlayerAI.players_ai[self.number] = self
    ...
    def move(self, step_size, ai_input):
        """AI decision to move"""
    ...
    def ai_decision(self, ai_input):
        ...
        sprite_positions[0] = self.rect.centerx / self.area.width
        sprite_positions[1] = self.rect.centery / self.area.height
        self.bonus_score[0] = self.bonus_score[1]  # Save bonus score from last call
        self.bonus_score[1] = 0  # Reset bonus score
        for i, enemy in enumerate(self.detection.scan(ai_input[0])):
            collision = self.detect_collision(enemy)  # +1 (true) or 0 (false)
            self.bonus_score[1] += collision
            sprite_positions[i + 1] = (enemy.x_pos - self.rect.centerx) / self.area.width  # Normalized enemy x pos
            sprite_positions[i + 2] = (enemy.y_pos - self.rect.centery) / self.area.height  # Normalized enemy y pos
            sprite_positions[i + 3] = collision
            sprite_positions[i + 4] = enemy.vx / enemy.speed_max
            sprite_positions[i + 5] = enemy.vy / enemy.speed_max
    ...
    def out_of_bound(self, contain=True, dangerous=False):
        ...
        if contain is True and dangerous is False:
            ...
            if not self.area.contains(self.rect):
                    self.image = PlayerAI.image[4]  # crash into wall (red boarder)
    ...
    def update(self, time_alive, ai_input=None, *args):
        """
        ...
        if self.remove:
            self.kill()  # Game over
        # -- Move player --
        self.move(self.step_size, ai_input)
        # -- Update score (i.e time alive) --
        if self.bonus_score[1] < self.bonus_score[0]:  # This check number of dodged enemies
            self.score += 10 * (self.bonus_score[0] - self.bonus_score[1])  # Added bonus
        self.score += 1
        # Edge cases
